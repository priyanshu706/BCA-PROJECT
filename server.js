...
// ---- Database ----
// Make sure the database is connected before any request is handled.
// This also runs the one-time seed on the very first request, which is
// needed on hosts that do not keep a server running all the time.
let seeded = false;
...
// ---- Start ----
// On a normal server (local machine, Render, Railway) we listen on a port.
// On a serverless host such as Vercel there is no port to listen on, so we
// only export the app and the platform calls it for each request.
